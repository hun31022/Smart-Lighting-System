...
//const WEATHER_KEY="ServiceKey=hbz2q3lKZqcICGX5TB0%2FQuXgmy6p%2BM58x6g87ksgi3eV4bSNbzr1mHFRNrWK4Hz5gOwMxdpZ%2BNc8nplynZg18A%3D%3D&";

//const AIR_KEY="ServiceKey=hbz2q3lKZqcICGX5TB0%2FQuXgmy6p%2BM58x6g87ksgi3eV4bSNbzr1mHFRNrWK4Hz5gOwMxdpZ%2BNc8nplynZg18A%3D%3D&";
...
//const RETURN_TYPE="";//리턴 타입 설정(json)
//동네예보조회 오퍼레이션--------------------------------------------------------------------------------------------
...
//const NOW_RETURN_TYPE="";//리턴 타입 설정(json)
//초단기실황조회 오퍼레이션--------------------------------------------------------------------------------------------
...
var tommorrow_pm10grade;
var tommorrow_pm25grade;
...
app.get("/", function(req, res) {
  ...
  http.get(today_dust_url, function(res){
    ...
    res.on('end', function(){
        ...
        if(body.split(' ')[0] == "<?xml") { console.log('get xml..'); return; }
      ...
      switch(result_body.list[0].pm25Grade1h)
      {
        ...
        case "3":
          ...
          break;
        case "4":
          today_pm25grade="매우나쁨";
          break;
      }
      console.log("현재 미세먼지 : " + today_pm10grade);
      console.log("현재 초미세먼지 : " + today_pm25grade);
    });
  });
  ...
  http.get(tomorrow_dust_pm10_url, function(res){
    ...
    res.on('end', function(){
      ...
      tommorrow_pm10grade = result_split[locate];
      console.log("내일 미세먼지(PM10) : " + tommorrow_pm10grade);
    });
  });
  ...
  http.get(tomorrow_dust_pm25_url, function(res){
    ...
    res.on('end', function(){
      ...
      var result=result_body.list[1].informGrade;
      var result_split=result.split(",");
      tommorrow_pm25grade = result_split[locate];
      console.log("내일 초미세먼지(PM2.5) : " +tommorrow_pm25grade);
    });
  });
  //단기실황(오늘 값)
  ...
  http.get(tomorrow_url, function(res){
      ...
      res.on('end', function(){
          ...
          for (i=0; i<count; i++)
          {
            if (result_body.response.body.items.item[i].fcstDate==20180310//tommorrow()
    ...
    aaa = String(today_pm10grade) + '___' + String(today_pm25grade) + '___' + String(tommorrow_pm10grade) + '___' + String(tommorrow_pm25grade) + '___' +String(tomorrow_hum) + '___' +  String(tomorrow_temp) + '___' +  String(tomorrow_weat) + '___' +  String(tomorrow_sky) + '___' +  String(today_hum) + '___' +  String(today_temp) + '___' +  String(today_weat) + '___' +  String(today_sky);
    
    //aaa = /*String(pm10grade) + '___' + String(pm25grade) + '___' + */String(tomorrow_hum) + '___' +  String(tomorrow_temp) + '___' +  String(tomorrow_weat) + '___' +  String(tomorrow_sky) + '___' +  String(today_hum) + '___' +  String(today_temp) + '___' +  String(today_weat) + '___' +  String(today_sky);
    res.json(aaa);
});
